...
const SayHello = ({ data = homeData.sayHelloSection }) => {
  const { main, social } = data;

  const handleWhatsApp = () => {
    window.open(main.whatsappUrl, "_blank");
  };

  const handleEmail = () => {
    window.location.href = `mailto:${main.emailAddress}`;
  };

  return (
    <section className="w-full bg-[#FAF8F5] py-16 px-4 flex flex-col items-center justify-center font-sans space-y-16">
      {/* Top Container: Say Hello */}
      <div
        className="
