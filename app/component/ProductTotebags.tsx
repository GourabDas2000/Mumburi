    ...
    <div className="group flex flex-col justify-between bg-white rounded-xl p-3.5 border border-stone-200 shadow-xs hover:shadow-lg transition-all duration-300">
      <Link href={`/stack/${productId}`} className="block w-full">
        ...
        <div className="w-full aspect-square bg-[#FAF8F5] mb-3 overflow-hidden rounded-xl relative p-1.5">
          <div
            className="w-full h-full bg-cover bg-center -rotate-3 group-hover:rotate-0 transition-transform duration-500"
            style={{
              clipPath: "polygon(50% 0%, 100% 86%, 0% 100%)",
              backgroundImage: `url(${product.cartImage || "/placeholder.jpg"})`,
            }}
          />
          <span className="absolute top-2 left-2 bg-[#8C4327] text-white font-sans text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded shadow-xs">
            {mainCategory}
          </span>
        </div>
