...
interface ProductHandPaintedTextilesProps {
  product?: Product;
}

export default function ProductHandPaintedTextiles({
  product = {},
}: ProductHandPaintedTextilesProps) {
  ...
  const mainCategory = product.productCategory?.[0] || "Hand Painted Textiles";
  ...
  return (
    <div className="group flex flex-col justify-between bg-white rounded-xl p-3.5 border border-stone-200 shadow-xs hover:shadow-lg transition-all duration-300">
      <Link href={`/stack/${productId}`} className="block w-full">
        {/* Shape: Horizontal Rectangle (Width > Height) */}
        <div className="w-full aspect-[4/3] bg-stone-100 rounded-lg overflow-hidden relative mb-3">
          <img
            src={product.cartImage || "/placeholder.jpg"}
            alt={product.productName || "Hand Painted Textile"}
            className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
          />
          <span className="absolute top-2 left-2 bg-[#4B3B68] text-white font-sans text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded shadow-xs">
            {mainCategory}
          </span>
        </div>
        ...
        <div className="flex flex-col mb-3">
          <h3 className="font-serif font-bold text-sm text-[#0f3d4c] group-hover:text-[#8C4327] transition-colors line-clamp-1">
            {product.productName || "Painted Textile"}
          </h3>
